...
class UMIInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        for i in range(history_length):
            ...
            image_dict[f"{i}_global_rgb"] = _parse_image(data[f"global_image_{i+1}"])
            image_dict[f"{i}_right_rgb"] = _parse_image(data[f"right_image_{i+1}"])
            image_dict[f"{i}_wrist_rgb"] = _parse_image(data[f"wrist_image_{i+1}"])


            image_mask_dict[f"{i}_rgb"] = np.True_
        image_dict['reference_image'] = _parse_image(data['reference_image'])
