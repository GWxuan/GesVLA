...
def preprocess_observation(
    ...
    *,
    ...
    for key in image_keys:
        ...
        #print(f"Image {i} stats - Mean: {img_array.mean()}, Max: {img_array.max()}, Min: {img_array.min()}")
        ...
        # jax.debug.print("📷 [preprocess]: shape={}, min={:.3f}, max={:.3f}, mean={:.3f}", 
        #                    image.shape, jnp.min(image), jnp.max(image), jnp.mean(image))
        out_images[key] = image

    # obtain mask
    #print("Debug: out_images:",out_images)
    out_masks = {}
    ...
    if not isinstance(observation, FuseObservation):
        ...
        fuseobservation = FuseObservation(
            images=out_images,
            image_masks=out_masks,
            state=observation.state,
            tokenized_prompt=observation.tokenized_prompt,
            ...
            origin_episode_idx=observation.origin_episode_idx,  # Episode index in training data for reasoning alignment.
            idx=observation.idx,  # Frame index within origin_episode_idx.
        )
